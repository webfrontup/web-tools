...
SnapSwiper.prototype.setScrollLeft = function (scrollLeft) {
	...
	if (!CSS.supports("scroll-behavior: smooth")) {
		var step = function () {
			var numScrolDistance =
				scrollLeft - _this.data.eleSwiperBox.scrollLeft;
...
SnapSwiper.prototype.automaticHandler = function () {
	...
	_this.data.timer = setInterval(function () {
		...
		var index =
			Math.floor(_this.data.eleSwiperBox.scrollLeft / clientWidth) + 1;
		...
		if (index > _this.data.imageList.length - 1) {
			...
			_this.data.eleSwiperBox.scrollLeft =
				clientWidth * index +
				_this.data.eleSwiperBox.offsetLeft * index;
		} else {
			_this.data.eleSwiperBox.style.scrollBehavior = "smooth";
			SnapSwiper.prototype.setScrollLeft(
				clientWidth * index + _this.data.eleSwiperBox.offsetLeft * index
			);
		}
	}, 3000);
};
