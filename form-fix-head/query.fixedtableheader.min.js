...
jQuery.fn.fixedtableheader = function (options) {
	...
	this.each(function (i) {
		//处理每个table（该插件使用class属性绑定，会存在多个table）
		var $tbl = $(this);
		...
		if ($tblhfixed.find(headerelement).length > 0) {
			...
			$(window).scroll(function () {
				...
				if (
					sctop > elmtop &&
					sctop <= elmtop + $tbl.height() - $tblhfixed.height()
				)
					$clonedTable.show();
				//原来的表头被滚动的不可见并且表格主体尚未被滚动出屏幕
