...
(function (factory) {
	...
	//【需实现】
	var BaseFolder =
		window.RecordAppBaseFolder ||
		/*=:=*/ "/Recorder/src/"; /*<@ "/Recorder/dist/" @>*/ //编译指令：源码时使用前面的文件夹，压缩时使用后面的文件夹
	...
	var Config_SupportPlatforms = [
		{
			...
			Config: {
				...
				*/
				...
				paths: [
					//当在app中使用此实现时，会自动把这些js全部加载
					{
						url: BaseFolder + "app-support/app-native-support.js",
						check: function () {
							return !Native.IsInit;
						},
					},
				],
			},
		...
		},
		{
			...
			Config: {
				...
				*/
				...
				paths: [
					//当在微信中使用此实现时，会自动把这些js全部加载
					{
						url:
							BaseFolder +
							"app-support/app-ios-weixin-support.js",
						check: function () {
							return !Weixin.IsInit;
						},
					},

					//amr解码引擎文件，因为微信临时素材接口返回的音频为amr格式，刚好有amr解码器，省去了服务器端的复杂性。amr解码器只是在Stop时才需要，因此可以在Stop前任何时候进行延迟加载
					{
						url: BaseFolder + "engine/beta-amr.js",
						lazyBeforeStop: 1,
						check: function () {
							return !Recorder.prototype.amr;
						},
					},
					/*=:=*/
					{
						url: BaseFolder + "engine/beta-amr-engine.js",
						lazyBeforeStop: 1,
						check: function () {
							return !Recorder.AMR;
						},
		...
		},
		{
			...
			Config: {
				paths: [
					//当使用默认实现时，会自动把这些js全部加载，如果core和编码器已手动加载，可以把此数组清空；另外需要其他编码格式的时候，直接把编码引擎加在后面（不需要mp3格式就删掉），会自动加载
					{
						url: BaseFolder + "recorder-core.js",
						check: function () {
					...
					/*=:=*/ {
						url: BaseFolder + "engine/mp3-engine.js",
						lazyBeforeStart: 1,
						check: function () {
							return !Recorder.lamejs;
						},
					} /*<@ @>*/, //编译指令：压缩后mp3-engine已包含在了mp3.js中
				],
			},
		},
	];
			...
			var tryLazyLoad = function (platform, first) {
				...
				if (!data) {
					...
					platform.OnLazyReady = function (fn) {
						startLoadEnd() && stopLoadEnd()
							? fn(data.msta || data.msto)
							: data.fall.push(fn);
					};
				}
				var startLoadEnd = function () {
					return data.usta == 1 || data.usta == 3;
				};
				var stopLoadEnd = function () {
					return data.usto == 1 || data.usto == 3;
				};
...
*/
		Start: function (set, success, fail) {
			...
			var checkMsg = checkRec.envCheck({
				envName: cur.Key,
				canProcess: cur.CanProcess(),
			});
			if (checkMsg) {
			...
			cur.LazyAtStart(function (err) {
				if (readyWait) {
					console.warn(
						"Start Wait Ready " + (Date.now() - readyWait) + "ms"
					);
				}
				readyWait = 1;
...
*/
		Stop: function (success, fail) {
			...
			cur.LazyAtStop(function (err) {
				if (readyWait) {
					console.warn(
						"Stop Wait Ready " + (Date.now() - readyWait) + "ms"
					);
				}
				readyWait = 1;
