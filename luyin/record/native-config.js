...
*/
		...
		var AppJsBridgeRequest = (window.AppJsBridgeRequest = function (
			action,
			args,
			call
		) {
			var p = GetParent();
			if (p != window && p.AppJsBridgeRequest) {
				//让iframe上层去处理，直到top层
...
// console.error(
// 	"【注意】本网站正在使用RecordApp的native-config.js测试用的配置例子，这个配置如果要使用到你的网站，需要自己重写或修改后才能使用"
// );
//别的站点引用弹窗醒目提示
if (
